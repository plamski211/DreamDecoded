...
React Native dream journaling app built with **Expo SDK 54** (new arch enabled), **expo-router v6** for file-based routing, **Zustand** for state management. Dreams are recorded via microphone, sent to **Google Gemini 2.5-flash** for analysis (transcription, mood extraction, symbol identification, interpretation), and persisted locally.
...
Auth guard in root `_layout.tsx` (`useProtectedRoute`) redirects based on session/onboarding state. App works offline-first with a default local user (no auth required).

### State (`lib/store.ts`)

Single Zustand store (`useAppStore`) holds auth state, recording state, processing state, dream list, subscription status, and current decoded dream. Access slices via selectors: `useAppStore((s) => s.fieldName)`. Store actions persist dreams to storage as a side effect.

### AI Integration (`lib/ai.ts`)

`processDream()` sends audio as base64 to Gemini with a structured prompt. Gemini returns JSON with title, transcription, summary, moods (with confidence scores), symbols (with meanings), and interpretation. `askDream()` handles multi-turn conversational follow-up about a dream, passing dream context (title, transcription, summary, interpretation) to Gemini.
...
Cross-platform: `expo-av` with HIGH_QUALITY preset + dB metering on native, `MediaRecorder` + Web Audio API frequency analysis on web. Real-time audio level callbacks (normalized 0-1) drive the breathing orb animation.
...
- **Web compatibility**: `Platform.OS === 'web'` checks throughout. Custom `AlertModal` component replaces `Alert.alert()` (which fails on web with 3+ buttons). `lib/storage.web.ts` replaces SQLite. `lib/notifications.ts` returns no-ops on web.
- **Animations**: React Native's built-in `Animated` API (no native dependencies). `FadeInView` component provides staggered fade-in-down entrance animations. `GlowOrb` uses `Animated.loop`/`Animated.sequence` for breathing scale + audio level reactivity.
