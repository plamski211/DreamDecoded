...
export default function SignUpScreen() {
  ...
  const showAlert = (title: string, message: string, buttons?: AlertConfig['buttons']) => {
    setAlertConfig({ title, message, buttons: buttons ?? [{ text: 'OK', style: 'default' }] });
  ...
  const handleSignUp = async () => {
    ...
    if (!data.session) {
      showAlert('Check Your Email', 'We sent a confirmation link to your email. Please confirm, then sign in.', [
        {
          text: 'Go to Sign In',
          style: 'default',
          onPress: () => router.replace({ pathname: '/(auth)/sign-in', params: { prefillEmail: email } }),
        },
      ]);
    }
  };
          ...
          <View style={styles.content}>
            ...
            <View style={styles.form}>
              <View style={styles.inputGroup}>
                ...
                <TextInput style={[styles.input, { backgroundColor: c.surface, borderColor: c.border, color: c.text, fontFamily: theme.fonts.body }]} placeholder="Your name" placeholderTextColor={c.textTertiary} autoCapitalize="words" autoComplete="name" textContentType="name" value={name} onChangeText={setName} />
              </View>
              <View style={styles.inputGroup}>
